    ...
    @if(!(Auth::user()->id == $post->author_id))
      @if((Auth::user()->channel == "channel2") && ($likecount[$post->id]['likes'] > 0))
                 <article data-postid='{{$post->id}}'>
                             @if (Storage::disk('local')->has(explode(' ',$user->name)[0] . '-' . $user->id . '.jpg'))
                             <img src="{{ route('account.image', ['filename' => explode(' ',$user->name)[0] . '-' . $user->id . '.jpg']) }}" alt="User" class="img-responsive" style="height:40px;">
                             @endif
                          <div class='info'>
        ...
        <article data-postid='{{$post->id}}'>
            <div class='info'>
            {{Auth::user()->username}} <br></div><div class="details">{{$post->created_at}}
            </div>
           <p class="postcont">{{$post->body }}</p>
             <div class='interaction'>
           <p>
